    ...
    let actorProp (mediator: IActorRef<Publish>) (mailbox: Eventsourced<_>) =
        ...
        let rec set (state: Order option * int) =
            actor {
                ...
                | Command { Command = MarkAsDelivered; CorrelationId = ci }, (Some (o), v) ->
                ...
                | Command { Command = PlaceOrder o; CorrelationId = ci }, (None, version) ->
                    ...
                    let event =
                        toEvent ci ({ o with Version = version + 1 } |> OrderPlaced) (version + 1)
                ...
                | Command { Command = PlaceOrder o; CorrelationId = ci }, (Some _, version) ->
                    ...
                    mailbox.Sender()
                    <! ((toEvent ci (OrderRejected(o, "duplicate")) version)
                        |> Event)
        ...
        let rec set (state: State) =
            ...
            actor {
                ...
                | :? (Common.Event<Delivery.Event>) as deliveryEvent, state ->
                    ...
                    | { Event = Delivery.NoDeliveries }, ProcessingOrder o -> deliveryActor <! startDeliveryCmd o
...
let init () =
    ...
    Order.init
    |> sprintf "Order initialized: %A"
    |> Log.Debug

    Delivery.init
    |> sprintf "Delivery initialized: %A"
    |> Log.Debug

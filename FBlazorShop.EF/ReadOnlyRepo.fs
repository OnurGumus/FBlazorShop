...
type ReadOnlyRepo<'T>(dataContext: PizzaStoreContext) =
    interface IReadOnlyRepo<'T> with
        member _.Queryable: IQueryable<'T> =
            if typeof<'T> = typeof<PizzaSpecial>
            then dataContext.Specials.AsQueryable() :?> IQueryable<'T>
            elif typeof<'T> = typeof<Topping>
            then dataContext.Toppings.AsQueryable() :?> IQueryable<'T>

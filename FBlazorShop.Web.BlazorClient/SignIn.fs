...
let signInCmd (remote: PizzaService) (email, pass) =
    let doWork (email, pass) =
        async { return! remote.signIn (email, pass) }

    Cmd.OfAsync.either doWork (email, pass) (function
        | Ok m -> SignInSuccessful(m)
        | Error s -> SignInFailed s) (fun m -> SignInFailed(m.ToString()))
...
let update remote message (model: Model) =
    ...
    let validate form =
        ...
        | None -> { model with CurrentLogin = form }
    ...
    | SignInSubmitted, { ValidatedLogin = None } -> model.CurrentLogin |> validateForced, Cmd.ofMsg (SignInSubmitted)

    | SignInSubmitted, _ -> model, signInCmd remote (model.CurrentLogin.Email, model.CurrentLogin.Password)
    | _ -> failwith ""
...
let view (model: Model) (dispatch: _ -> unit) =
    ...
    | true ->
        ...
        let formFieldItem =
            Bolero.F.formFieldItem model.ValidatedLogin model.Focus focused

        let pd name =
            Action<string>(fun v -> dispatch (SetFormField(name, v)))
        ...
        SignIn().FormItems(formItems).Cancel(fun _ -> dispatch SignInCancelled)
            .Submit(fun _ -> dispatch SignInSubmitted).LoginError(defaultArg model.FailureReason "").Elt()
    | _ -> empty

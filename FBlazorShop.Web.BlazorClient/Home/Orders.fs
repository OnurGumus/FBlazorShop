...
let getPizzas (jsRuntime: IJSRuntime) =
    let doWork () =
        async {
            let! res =
                jsRuntime.InvokeAsync<string>("window.localStorage.getItem", "pizzas").AsTask()
                |> Async.AwaitTask
            ...
            return
                ...
                | t ->
                    t
                    |> JsonConvert.DeserializeObject<Pizza list>
                    |> PizzasLoaded
        }
...
let updatePizzaList (jsRuntime: IJSRuntime) pizzas =
    let doWork () =
        async {
            ...
            do! jsRuntime.InvokeVoidAsync("window.localStorage.setItem", "pizzas", pizzaSer).AsTask()
                |> Async.AwaitTask
...
let init jsRuntime = { Order = None }, Cmd.none
...
type OrderView() =
    ...
    override this.OnAfterRenderAsync(firstRender) =
        ...
        async {
            do! res
            if firstRender then this.Dispatch Rendered
            return ()
        }
    ...
    override _.View state dispatcher =
        ...
        let cartItem (index, pizza: Pizza) =
            div [ attr.``class`` "cart-item" ] [
                a [
                    on.click (fun _ -> index |> PizzaRemoved |> dispatcher)
                    attr.``class`` "delete-item"
                  ] [
                    text "x"
                ]
                div [ attr.``class`` "title" ] [
                    textf "%s\" %s" (pizza.Size.ToString()) pizza.Special.Name
                ]
                ul [] [
        ...
        let lower =
            ...
            | Some order ->
                div [ attr.``class`` "order-total" ] [
                    text "Total:"
                    span [ attr.``class`` "total-price" ] [
                        text (order.FormattedTotalPrice)
                    ]
                    button [
                               attr.``class`` "btn btn-warning"
                               on.click (fun _ -> order |> CheckoutRequested |> dispatcher)
                           ] [
                        text "Order >"
                    ]
                ]

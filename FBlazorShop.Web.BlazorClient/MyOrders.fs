...
type Model =
    {
        MyOrders: OrderWithStatus list option
    }
...
let update (remote: PizzaService) message (model, commonState: Common.State) =
    ...
    | Initialized, Common.AuthState.Success auth ->
        model, Cmd.OfAsync.either remote.getOrderWithStatuses auth.Token OrdersLoaded raise, Cmd.none
        ...
        div [ attr.``class`` "main" ] [
            ...
            | Some orders ->
                let viewOrder (s: OrderWithStatus) =
                    OrderList.OrderItem().OrderCreatedTime(s.Order.CreatedTime.ToLongDateString())
                             .OrderFormattedTotalPrice(s.Order.FormattedTotalPrice)
                             .OrderOrderId(s.Order.OrderId.ToString())
                             .OrderPizzasCount(s.Order.Pizzas.Length.ToString()).StatusText(s.StatusText)
                             .Version(s.Order.Version.ToString()).Elt()

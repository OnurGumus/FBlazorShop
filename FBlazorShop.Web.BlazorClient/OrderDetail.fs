...
let loadPeriodically remote token id version firstTime =
    let doWork i =
        ...
        async {
            if not firstTime then do! Async.Sleep 4000
            return! getOrder 100
...
let init (id, version) =
    { Order = None; Key = ""; Version = 0 }, Cmd.ofMsg (OrderLoaded(id, version, None, true))
...
let update remote message (model: Model, commonModel: Common.State) =
    ...
    | OrderLoaded (id, version, order, firstTime), Common.AuthState.Success auth ->
        ...
        loadPeriodically remote auth.Token id version firstTime,
        Cmd.none
    | _ -> failwith ""
...
type OrderDetailView() =
    ...
    override _.View model dispatch =
        div [ attr.``class`` "main" ] [
            ...
            <| function
            | Some x ->
                OrderDetail().OrderCreatedTimeToLongDateString(x.Order.CreatedTime.ToLongDateString())
                    .StatusText(x.StatusText).OrderReview(OrderReview.view x.Order dispatch).Map(map (x.MapMarkers))
                    .Elt()
...
let view (model: Model) dispatch =
    ecomp<OrderDetailView, _, _> [] model dispatch

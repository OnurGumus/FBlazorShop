...
let update remote message (model: Model, commonState: Common.State) =
    ...
    let validateModelForAddressForced address =
        ...
        { model with
            CurrentAddress = address
            ValidatedAddress = Some vAddress
        }

    let validateModelForAddress address =
        match model.ValidatedAddress with
        | None -> { model with CurrentAddress = address }
    ...
    | OrderPlaced order, ({ ValidatedAddress = None }, _) ->
        ...
        |> validateModelForAddressForced,
        Cmd.ofMsg (OrderPlaced order),
        Cmd.none
    ...
    | OrderPlaced order, (_, Common.AuthState.Success auth) ->
        let order =
            { order with
                DeliveryAddress = model.CurrentAddress
            }

        let cmd =
            Cmd.OfAsync.either remote.placeOrder (auth.Token, order) (function
                | Ok (id, v) -> OrderAccepted(id, v)
                | Error e -> invalidOp e) raise
...
let view (model: Model) dispatch =
    div [ attr.``class`` "main" ] [
        ...
        | Some currentOrder ->
            ...
            let formFieldItem =
                Bolero.F.formFieldItem model.ValidatedAddress model.Focus focused "text"
            ...
            Checkout().OrderReview(OrderReview.view currentOrder dispatch)
                .PlaceOrder(fun _ -> dispatch (OrderPlaced currentOrder)).FormFieldItems(formItems).Elt()

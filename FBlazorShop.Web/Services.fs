...
type public PizzaService(ctx: IRemoteContext) =
    ...
    let generateToken email =
        ...
        let algorithm = JWT.Algorithms.HMACSHA512Algorithm()

        builder.WithAlgorithm(algorithm).WithSecret(secret)
               .AddClaim("exp", System.DateTimeOffset.UtcNow.AddDays(7.0).ToUnixTimeSeconds()).AddClaim(EMAIL, email)
               .Encode()

    let extractUser token =
        let json =
            JwtBuilder().WithAlgorithm(HMACSHA512Algorithm()).WithSecret(secret).MustVerifySignature().Decode<IDictionary<string, string>>
                (token)
    ...
    member private this.GetItems<'T> (filter: Expression<System.Func<'T, bool>> option) take skip () =
        ...
        async {
            let! b =
                ...
                q |> repo.ToListAsync |> Async.AwaitTask
            ...
            getOrderWithStatuses =
                fun token ->
                    ...
                    async {
                        ...
                        let! orders = this.GetItems<OrderEntity> filter None None ()
                        let orders = orders |> List.map Projection.toOrder
